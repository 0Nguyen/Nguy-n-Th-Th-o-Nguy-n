...
def render_header():
    pill_1 = _u("🧭 Bộ lọc phân tích", "🧭 Analysis filter")
    pill_2 = _u("📘 Báo cáo dễ đọc", "📘 Easy-read report")
    st.markdown(
        f"""
...
def render_sidebar_tools():
    ...
    if not st.session_state.get("analysis_tools_visible", False):
        st.sidebar.caption(
            _u(
                "Bộ công cụ sẽ hiện sau khi bạn xác nhận ánh xạ và bắt đầu phân tích.",
    ...
    st.sidebar.caption(
        _u(
            "Chế độ mặc định: bệnh nhân có bảo hiểm + chỉ định ngoài bảo hiểm. Bộ lọc thủ công cho phép xem nhóm khác, nhưng phải đọc kết quả đúng theo scope đã chọn.",
