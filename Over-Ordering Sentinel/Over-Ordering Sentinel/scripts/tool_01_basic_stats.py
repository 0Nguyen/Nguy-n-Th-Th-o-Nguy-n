...
def _group_by_doctor(denominator_df: pd.DataFrame, numerator_df: pd.DataFrame) -> pd.DataFrame:
    ...
    table["review_note"] = table.apply(
        lambda row: "Có tín hiệu cần xem trong scope hiện tại." if row["numerator_orders"] > 0 else "Không có tín hiệu trong scope hiện tại.",
        axis=1,
    )
...
def run(df, context):
    ...
    overview_table = pd.DataFrame(
        [
            ...
            {"Chi so / Metric": "Current preset / Bộ lọc hiện tại", "Gia tri / Value": scope.preset_name},
            ...
            {"Chi so / Metric": "Denominator rows / Số dòng mẫu nền", "Gia tri / Value": denominator_rows},
            {"Chi so / Metric": "Numerator rows / Số dòng tín hiệu", "Gia tri / Value": numerator_rows},
            {"Chi so / Metric": "Numerator rate / Tỷ lệ tín hiệu", "Gia tri / Value": format_pct(numerator_rate)},
    ...
    return {
        "tool_name": "01 - Tổng quan mẫu phân tích",
        "status": "completed",
        "summary": summary,
        "tables": result_tables,
        "notes": [
            f"Scope preset: {scope.preset_name}. Denominator rows: {denominator_rows}. Numerator rows: {numerator_rows}.",
            "Kết quả này chỉ hỗ trợ rà soát cho cohort đã chọn, không phải kết luận sai phạm.",
        ],
    }
