...
def run(df, context):
    ...
    if denominator_df.empty:
        empty = pd.DataFrame()
        return {
            "tool_name": "03 - Dịch vụ và chi phí",
            "status": "completed",
            ...
            "notes": ["Không có dòng mẫu nền để rà dịch vụ trong scope hiện tại."],
        }
    ...
    return {
        ...
        "notes": [
            ...
            "Bảng này chỉ làm nổi bật tập trung dịch vụ và mẫu chi phí để hội đồng xem tiếp, không phải kết luận sai phạm.",
        ],
    }
