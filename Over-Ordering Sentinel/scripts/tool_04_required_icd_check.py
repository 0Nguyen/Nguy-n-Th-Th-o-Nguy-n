...
def _build_flag(row: pd.Series, rule: ICDRule, mismatch_type: str, severity: str, note: str, context_keyword: str = "") -> dict[str, Any]:
    return {
        ...
        "recommended_review_action": "Check the ICD, orders, and clinical context again before making a decision.",
    }
...
def run(df, context):
    ...
    if review_df.empty:
        empty = pd.DataFrame()
        return {
            "tool_name": "04 - ICD and order check",
            "status": "completed",
            "summary": {
                "flag_count": 0,
            ...
            "notes": ["No rows were available in the current scope for ICD / order review."],
        }
        ...
        if not has_icd and context_status == "NO_CONTEXT_FOUND":
            flags.append(
                _build_flag(
                    ...
                    "Missing ICD and no matching clinical context was found.",
                    matched_context,
                )
            )
            continue
        ...
        if is_generic and context_status == "NO_CONTEXT_FOUND":
            flags.append(
                _build_flag(
                    ...
                    "WEAK_ICD",
                    "YELLOW",
                    "ICD/diagnosis is too generic or weak to clearly explain the order.",
                    matched_context,
                )
            )
            continue
        ...
        if has_icd and not has_expected_icd and context_status == "NO_CONTEXT_FOUND":
            flags.append(
                _build_flag(
                    ...
                    "The current ICD does not match the order group in the audit rule.",
                    matched_context,
                )
            )
            continue
        ...
        if context_status == "PARTIALLY_RESOLVED_WEAK_CONTEXT" and not has_expected_icd:
            flags.append(
                _build_flag(
                    ...
                    "WEAK_ICD",
                    "YELLOW",
                    "There is weak context, but the ICD is still not strong enough to explain the order.",
                    matched_context,
                )
            )
            continue
        ...
        if has_expected_icd and context_status == "NO_CONTEXT_FOUND" and covered_status == "out_of_insurance" and amount_value >= high_cost_threshold:
            flags.append(
                _build_flag(
                    ...
                    "This row has an unclear ICD/context pair within a high-cost out-of-insurance group.",
                    matched_context,
                )
            )
            ...
            generic_rows.append(
                {
                    ...
                    "recommended_review_action": "Review the repeated use of a generic ICD across multiple services.",
                }
            )
    ...
    if required_icd_flags.empty:
        ...
        return {
            ...
            "notes": [
                "No ICD / order mismatches were found in the selected scope.",
                "If the ICD column is sparse, the app still continues and records notes instead of stopping.",
            ],
        }
    ...
    if mismatch_rows.empty:
        ...
        doctor_icd_mismatch_summary["explanation_summary"] = doctor_icd_mismatch_summary.apply(
            lambda row: (
                f"{row['doctor']} has {int(row['mismatch_count'])} ICD/context signals to review; "
                f"missing ICD={int(row['missing_icd_count'])}, weak ICD={int(row['weak_icd_count'])}, "
    ...
    return {
        ...
        "notes": [
            "An ICD / order match is only review evidence, not a finding of wrongdoing.",
            "Repeated generic ICD usage and context-only matches are separated so the committee can review them in context.",
        ],
    }
