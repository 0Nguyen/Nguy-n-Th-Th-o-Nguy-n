...
def _build_doctor_frame(denominator_df: pd.DataFrame, numerator_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return {
        "tool_name": "02 - Doctor review ranking",
        "status": "completed",
        "summary": summary,
        "tables": {
        ...
        "notes": [
            ...
            "This score is only for review prioritization and must not be read as a finding of wrongdoing.",
        ],
    }
...
def run(df, context):
    ...
    if denominator_df.empty:
        ...
        "tool_name": "02 - Doctor review ranking",
            ...
            "notes": ["No baseline rows were available to rank doctors in the current scope."],
        }
