...
def _make_review_question(
    ...
    if target_type == "doctor":
        return (
            f"Review the record for {doctor}: which orders are generating the signal, "
            f"whether the ICD/context is reasonable, and whether the pattern differs from the department baseline."
        )
    if target_type == "procedure":
        return (
            f"Review procedure {procedure}: check why this service is generating many signals "
            f"or high costs in the current analysis group."
        )
    if target_type == "icd_context":
        return f"Check ICD {icd} for procedure {procedure}: does the ICD/context sufficiently explain the order?"
    if target_type == "case":
        return f"Check this case: {reason}"
    return "Have the review committee re-check the evidence before drawing any conclusion."
...
def _extract_top_doctors(tables: dict[str, Any], limit: int = 10) -> pd.DataFrame:
    ...
    for _, row in source.iterrows():
        ...
        reason_parts = []
        if numerator_orders:
            reason_parts.append(f"{int(numerator_orders)} signals")
        if numerator_rate:
            reason_parts.append(f"rate {numerator_rate}")
        if ratio:
            reason_parts.append(f"{ratio}x the department baseline")
        if amount:
            reason_parts.append(f"amount {_format_vnd(amount)}")
        if top_procedure:
            reason_parts.append(f"top procedure: {top_procedure}")
        if top_icd:
            reason_parts.append(f"common ICD: {top_icd}")

        why = "; ".join(reason_parts) if reason_parts else "There are signals that need committee review."
...
def _extract_top_procedures(tables: dict[str, Any], limit: int = 10) -> pd.DataFrame:
    ...
    for _, row in source.iterrows():
        ...
        why_parts = []
        if numerator_orders:
            why_parts.append(f"{int(numerator_orders)} signals")
        if amount:
            why_parts.append(f"amount {_format_vnd(amount)}")
        if high_cost.lower() in {"true", "1", "yes"}:
            why_parts.append("high-cost procedure")
        if sensitive.lower() in {"true", "1", "yes"}:
            why_parts.append("sensitive procedure")

        why = "; ".join(why_parts) if why_parts else "This procedure has signals that need review."
...
def _extract_icd_cases(tables: dict[str, Any], limit: int = 20) -> pd.DataFrame:
    ...
    for _, row in source.iterrows():
        ...
        why = f"{mismatch_type}. ICD={icd}; diagnosis={diagnosis or 'N/A'}; note={note or 'committee should re-check the ICD/context'}"
...
def _make_plain_language_summary(final_report: dict[str, Any], action_table: pd.DataFrame) -> list[str]:
    ...
    lines = [
        f"The app is analyzing: {preset_name}.",
        f"The baseline has {denominator_rows} rows; {numerator_rows} rows are review signals.",
        f"Estimated total signal value: {numerator_amount}.",
    ]
    ...
    if not action_table.empty:
        ...
        lines.append(f"First review priority: {target} ({target_type}) because {why}.")
        ...
        if not case_rows.empty:
            ...
            lines.append(
                "Case evidence to review: "
                f"doctor {case.get('review_target', 'N/A')}, "
                f"procedure {case.get('procedure', 'N/A')}, "
                f"date {case.get('review_date', 'N/A')}, "
                f"claim {case.get('claim_id', 'N/A')}."
            )
    else:
        lines.append("There is not enough data to generate an automatic review priority list.")

    lines.append("The result is a review signal, not a finding of wrongdoing or fraud.")
    return lines
...
def build_judge_summary(final_report: dict[str, Any], max_rows: int = 25) -> dict[str, Any]:
    ...
    how_to_read = [
        "Tool 01 = cohort size and signals.",
        "Tool 02 = which doctor to review first.",
        "Tool 03 = which procedure stands out by cost or frequency.",
        "Tool 04 = which ICD/context cases are missing, weak, or mismatched.",
        "Tool 05 = which cases have enough context to reduce false alarms.",
        "Review priority is a review priority, not evidence of wrongdoing.",
    ]
